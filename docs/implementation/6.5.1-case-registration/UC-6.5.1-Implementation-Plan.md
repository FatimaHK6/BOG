...
7. [المرحلة 5: واجهة المستخدم (Frontend)](#7-المرحلة-5-واجهة-المستخدم-frontend)
8. [ملخص الملفات](#8-ملخص-الملفات)
9. [التحقق](#9-التحقق)
...
| المرحلة 5 | واجهة المستخدم (Angular Frontend) | المرحلة 4 (API جاهزة) |
...
## 7. المرحلة 5: واجهة المستخدم (Frontend)

**التقنيات:** Angular 13 + Angular Material + RTL Support
...
|--------|-------|
| الإطار | Angular 13.3 |
| مكتبة UI | Angular Material |
| الاتجاه | RTL (من اليمين لليسار) |
| اللغة | العربية (أساسي) + الإنجليزية |
| التصميم | متجاوب (Desktop, Tablet, Mobile) |

### 7.2 المتطلبات والتبعيات
...
│   ├── services/
│   │   ├── api.service.ts                   # خدمة HTTP الأساسية
│   │   └── auth.service.ts                  # خدمة المصادقة
│   ├── interceptors/
│   │   ├── auth.interceptor.ts              # إضافة Token للطلبات
│   │   └── error.interceptor.ts             # معالجة الأخطاء
│   └── guards/
│       └── auth.guard.ts                    # حماية المسارات
│
├── shared/                                  # المكونات المشتركة
│   ├── components/
│   │   ├── confirmation-dialog/             # حوار التأكيد
│   │   ├── loading-spinner/                 # مؤشر التحميل
│   │   ├── address-form/                    # نموذج العنوان
│   │   ├── identity-lookup/                 # البحث بالهوية (أبشر)
│   │   └── file-upload/                     # رفع الملفات
│   ├── directives/
│   │   └── rtl.directive.ts                 # توجيه RTL
│   └── pipes/
│       ├── hijri-date.pipe.ts               # تحويل التاريخ الهجري
│       └── arabic-number.pipe.ts            # الأرقام العربية
│
├── features/                                # الوحدات الوظيفية
│   └── case-registration/                   # وحدة قيد الدعوى
...
│       │   └── case-request.service.ts
│       │
│       ├── pages/                           # الصفحات الرئيسية
│       │   ├── case-request-list/           # قائمة الطلبات
│       │   └── case-request-detail/         # تفاصيل الطلب
│       │
│       └── components/                      # المكونات
...
│           │   ├── plaintiff-list/
│           │   ├── plaintiff-form/
│           │   └── plaintiff-wizard/        # معالج 4 خطوات
│           │
...
### 7.4 المكونات الرئيسية

#### 7.4.1 صفحة تفاصيل الطلب (Case Request Detail)

```typescript
// case-request-detail.component.ts
@Component({
  selector: 'app-case-request-detail',
  templateUrl: './case-request-detail.component.html'
})
export class CaseRequestDetailComponent implements OnInit {
  requestId: number;
  currentSection: string = 'plaintiffs';

  sections = [
    { id: 'plaintiffs', label: 'المدعين', icon: 'person' },
    { id: 'defendants', label: 'المدعى عليه', icon: 'people' },
    { id: 'case-data', label: 'بيانات الدعوى', icon: 'description' },
    { id: 'additional-info', label: 'معلومات إضافية', icon: 'info' },
    { id: 'deficiencies', label: 'النواقص', icon: 'warning' },
    { id: 'finish', label: 'إنهاء الطلب', icon: 'check_circle' }
  ];
}
```

#### 7.4.2 معالج إضافة المدعي (4 خطوات)
...
export class PlaintiffWizardComponent implements OnInit {
  ...
  steps = [
    { number: 1, label: 'بيانات الشخصي' },
    { number: 2, label: 'بيانات الممثلين' },
    { number: 3, label: 'المرفقات' },
    { number: 4, label: 'بيانات إضافية' }
  ];

  onPlaintiffTypeChange(typeId: number): void {
    // تغيير حقول النموذج حسب نوع المدعي
  }

  lookupAbsher(identityNumber: string): void {
    // جلب البيانات من أبشر
  }
}
```

#### 7.4.3 مكون البحث بالهوية (Absher Lookup)
...
export class IdentityLookupComponent {
  @Input() identityTypes: IdentityType[];
  @Output() personData = new EventEmitter<AbsherPersonData>();
  @Output() addressData = new EventEmitter<AbsherAddressData>();
  ...
  lookup(): void {
    this.isLoading = true;
    this.absherService.getPersonData(this.identityNumber, this.identityType)
      .subscribe(data => {
...
export interface Plaintiff {
  ...
  attachments?: PlaintiffAttachment[];
...
export class PlaintiffService {
  ...
  // قائمة المدعين للطلب
  getPlaintiffs(requestId: number): Observable<Plaintiff[]> {
    return this.http.get<Plaintiff[]>(`${this.baseUrl}/case-requests/${requestId}/plaintiffs`);
  }

  // تفاصيل مدعي
  getPlaintiff(id: number): Observable<Plaintiff> {
    return this.http.get<Plaintiff>(`${this.baseUrl}/plaintiffs/${id}`);
  }

  // إضافة مدعي
  createPlaintiff(requestId: number, plaintiff: PlaintiffCreateDto): Observable<Plaintiff> {
    return this.http.post<Plaintiff>(`${this.baseUrl}/case-requests/${requestId}/plaintiffs`, plaintiff);
  }

  // تعديل مدعي
  updatePlaintiff(id: number, plaintiff: PlaintiffUpdateDto): Observable<Plaintiff> {
    return this.http.put<Plaintiff>(`${this.baseUrl}/plaintiffs/${id}`, plaintiff);
  }

  // حذف مدعي
  deletePlaintiff(id: number): Observable<void> {
    return this.http.delete<void>(`${this.baseUrl}/plaintiffs/${id}`);
  }

  // تعيين كمقدم طلب
  setAsApplicant(id: number): Observable<void> {
    return this.http.post<void>(`${this.baseUrl}/plaintiffs/${id}/set-applicant`, {});
  }
}
...
export class AbsherService {
  private baseUrl = '/api/integration/absher';

  constructor(private http: HttpClient) {}

  verifyIdentity(identityNumber: string, identityType: number): Observable<AbsherVerificationResult> {
    return this.http.post<AbsherVerificationResult>(`${this.baseUrl}/verify`, {
      identityNumber,
      identityType
    });
  }

  getPersonData(identityNumber: string, identityType: number): Observable<AbsherPersonData> {
    return this.http.get<AbsherPersonData>(`${this.baseUrl}/person/${identityNumber}?type=${identityType}`);
  }

  getNationalAddress(identityNumber: string): Observable<AbsherAddressData> {
    return this.http.get<AbsherAddressData>(`${this.baseUrl}/address/${identityNumber}`);
  }
}
```

### 7.7 المسارات (Routing)
...
const routes: Routes = [
  {
    path: '',
    component: CaseRequestListComponent
  },
  {
    path: 'new',
  ...
  },
  {
    ...
    component: CaseRequestDetailComponent,
    children: [
      { path: '', redirectTo: 'plaintiffs', pathMatch: 'full' },
      { path: 'plaintiffs', component: PlaintiffListComponent },
      { path: 'plaintiffs/new', component: PlaintiffWizardComponent },
      { path: 'plaintiffs/:plaintiffId', component: PlaintiffWizardComponent },
      { path: 'defendants', component: DefendantListComponent },
      { path: 'defendants/new', component: DefendantFormComponent },
      { path: 'defendants/:defendantId', component: DefendantFormComponent },
      { path: 'case-data', component: CaseDataComponent },
      { path: 'additional-info', component: AdditionalInfoComponent },
      { path: 'deficiencies', component: DeficienciesComponent },
      { path: 'finish', component: FinishRequestComponent }
    ]
  }
];
...
### 7.8 رسائل التحقق (Validation Messages)
...
export const VALIDATION_MESSAGES = {
  ...
  // أخطاء المرفقات
  BR04_FORMAT: 'يجب أن يكون الملف بصيغة PDF فقط',
  BR04_SIZE: 'حجم الملف يتجاوز الحد الأقصى (4 ميجابايت)',

  // أخطاء مقدم الطلب
  BC07: 'فقط المدعي من نوع "فرد" يمكنه أن يكون مقدم الطلب',

  // رسائل النجاح
  SUCCESS_PLAINTIFF_ADDED: 'تم إضافة المدعي بنجاح',
  SUCCESS_PLAINTIFF_UPDATED: 'تم تحديث بيانات المدعي بنجاح',
  SUCCESS_PLAINTIFF_DELETED: 'تم حذف المدعي بنجاح',
  SUCCESS_REPRESENTATIVE_ADDED: 'تم إضافة الممثل بنجاح',
  SUCCESS_ATTACHMENT_UPLOADED: 'تم رفع المرفق بنجاح',
  SUCCESS_APPLICANT_SET: 'تم تعيين مقدم الطلب بنجاح',
  SUCCESS_REQUEST_SUBMITTED: 'تم تقديم الطلب بنجاح'
};
```

### 7.9 التصميم المتجاوب

```scss
// styles.scss - RTL & Responsive
:root {
...
### 7.10 ملخص الملفات (Frontend)

| الفئة | العدد | الملفات |
|-------|-------|---------|
| الخدمات الأساسية | 4 | api, auth, error-interceptor, auth-guard |
| المكونات المشتركة | 5 | confirmation-dialog, loading, address-form, identity-lookup, file-upload |
| صفحات | 2 | case-request-list, case-request-detail |
| مكونات المدعين | 3 | plaintiff-list, plaintiff-form, plaintiff-wizard |
| مكونات المدعى عليهم | 2 | defendant-list, defendant-form |
| مكونات الممثلين | 2 | representative-list, representative-form |
| مكونات المرفقات | 2 | attachment-list, attachment-upload |
| مكونات بيانات الدعوى | 3 | claims-list, related-cases, classifications |
| مكونات الإجراءات | 2 | submit-request, deficiencies |
| النماذج | 5 | plaintiff, defendant, representative, attachment, case-request |
| الخدمات | 6 | plaintiff, defendant, representative, attachment, case-request, absher |
| **الإجمالي** | **~36** | ملفات TypeScript/HTML/SCSS |
...
2. **إنشاء الهيكل**
   ```bash
   ng generate module features/case-registration --routing
...
3. **إنشاء المكونات المشتركة**
   ...
   ng generate component shared/components/address-form
   ng generate component shared/components/identity-lookup
   ng generate component shared/components/file-upload
   ```

4. **إنشاء مكونات قيد الدعوى**
   ```bash
   ng generate component features/case-registration/pages/case-request-list
   ng generate component features/case-registration/pages/case-request-detail
   ng generate component features/case-registration/components/plaintiff-list
   ng generate component features/case-registration/components/plaintiff-wizard
   # ... باقي المكونات
   ```

5. **إنشاء الخدمات**
   ```bash
   ng generate service features/case-registration/services/plaintiff
   ng generate service features/case-registration/services/defendant
   ng generate service features/case-registration/services/representative
   ng generate service features/case-registration/services/case-request
   ng generate service core/services/absher
   ```

### 7.12 مرجع UX

للاطلاع على مواصفات واجهة المستخدم التفصيلية:
- **ملف Markdown:** `docs/implementation/6.5.1-case-registration/Ux/UC-6.5.1-UX-Specification.md`
- **ملف HTML (للطباعة):** `docs/implementation/6.5.1-case-registration/Ux/UC-6.5.1-UX-Specification.html`
...
### المرحلة 5 (واجهة المستخدم - Frontend)
- 1 وحدة Angular (Case Registration Module)
- 4 خدمات أساسية (core)
- 5 مكونات مشتركة (shared)
- 2 صفحات رئيسية
- 3 مكونات للمدعين
- 2 مكونات للمدعى عليهم
- 2 مكونات للممثلين
- 2 مكونات للمرفقات
- 3 مكونات لبيانات الدعوى
- 2 مكونات للإجراءات
- 5 نماذج (models)
- 6 خدمات (services)
- **الإجمالي:** ~36 ملف TypeScript/HTML/SCSS
